...
export const fetchExperienceDetails = createAsyncThunk(
	"experiences/fetchExperienceDetails",
	async ({ userId, expId }, thunkAPI) => {
		try {
			const response = await fetch(`${BASE_URL}${userId}/experiences/${expId}`, {
				method: "GET",
...
const experiencesSlice = createSlice({
	...
	reducers: {
		...
		setCurrentExpId: (state, action) => {
			state.currentExpId = action.payload;
		},
	},
	extraReducers: (builder) => {
		builder
			...
			.addCase(editExperience.fulfilled, (state, action) => {
				const index = state.items.findIndex((exp) => exp._id === action.meta.arg.expId);
				if (index !== -1) {
